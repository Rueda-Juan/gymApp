...
import { useActiveWorkout, useWorkoutDb } from "@/entities/workout";
import { useRestTimer } from "../model/useRestTimer";
import { getExerciseName, useExerciseDb } from "@/entities/exercise";
...
import type { Exercise, WeightSuggestion, WorkoutExerciseState, WorkoutSetState } from "@kernel";
import type { ActiveWorkoutStore } from "@/entities/workout";
import type { RestTimerStore } from "../model/useRestTimer";
...
export interface UseActiveWorkoutControllerProps {
  ...
  supersetGroupExercises?: WorkoutExerciseState[];
}
...
export function useActiveWorkoutController(
  ...
  // STORE
  // ---------------------------
  const exercises = useActiveWorkout((s: ActiveWorkoutStore) => s.exercises);
  const currentExerciseIndex = useActiveWorkout(
    (s: ActiveWorkoutStore) => s.currentExerciseIndex,
  );
  const addSetStore = useActiveWorkout((s: ActiveWorkoutStore) => s.addSet);
  const addExercise = useActiveWorkout((s: ActiveWorkoutStore) => s.addExercise);
  const setCurrentExercise = useActiveWorkout((s: ActiveWorkoutStore) => s.setCurrentExercise);
  ...
  const [weightSuggestion, setWeightSuggestion] =
    useState<WeightSuggestion | null>(null);
  ...
  useEffect(() => {
    ...
    workoutService
      .suggestWeight(activeExerciseId)
      .then((s: WeightSuggestion | null) => !cancelled && setWeightSuggestion(s))
  ...
  const shouldStartRestTimerAfterSet = useMemo(() => {
    ...
    return (
      supersetOrder[supersetCarouselIndex] ===
  ...
  const onSetToggle = useCallback(
    async (exerciseId: string, setId: string, completed: boolean) => {
      const ex = exercises.find((e: WorkoutExerciseState) => e.id === exerciseId);
      if (!ex) return;
  ...
  const handleAddExerciseSelection = useCallback(
    (item: Exercise) => {
      addExercise({
        id: createClientId(),
        exerciseId: item.id,
  ...
  const openPlateCalculator = useCallback((sets: WorkoutSetState[]) => {
