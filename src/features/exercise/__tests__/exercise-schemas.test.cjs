const { CreateCustomExerciseSchema, LoadTypeSchema } = require('../utils/helpers');

describe('LoadTypeSchema', () => {
  ['weighted', 'bodyweight', 'assisted', 'timed'].forEach((type) => {
    test(`accepts valid load type: ${type}`, () => {
      expect(LoadTypeSchema.safeParse(type).success).toBe(true);
    });
  });

  test('rejects unknown load type', () => {
...
describe('CreateCustomExerciseSchema', () => {
  ...
  test('accepts a valid custom exercise input', () => {
