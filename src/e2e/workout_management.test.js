describe('Gestión del Entrenamiento', () => {
  ...
  it('agrega un ejercicio a un entrenamiento activo', async () => {
    ...
    await expect(element(by.id('workout-exercise-item')).atIndex(0)).toBeVisible();
  });
  ...
  it('navega al siguiente y anterior ejercicio', async () => {
    ...
    await expect(element(by.id('active-exercise-title'))).toHaveText('Sentadilla');

    await element(by.id('prev-exercise-button')).tap();
    await expect(element(by.id('active-exercise-title'))).toHaveText('Press de Banca');
  });
