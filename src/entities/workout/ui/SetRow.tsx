...
import { TextInput, Keyboard, useWindowDimensions, StyleSheet } from 'react-native';
...
import { useSetRowAnimation } from '@/shared/ui/hooks/useSetRowAnimation';
import { parseWeight } from '@/shared/lib/formatters';
...
export interface SetRowFeedback {
  light: () => void;
  medium: () => void;
  heavy: () => void;
  success: () => void;
...
export const SetRow = React.memo(function SetRow({
  ...
  const weightIncrement = (availablePlates?.length ?? 0) > 0
    ? Math.min(...availablePlates!)
    : 2.5;
  ...
  return (
    <YStack width="100%" borderRadius="$lg" overflow="hidden" testID={`set-row-${index}`}>
      ...
      <GestureDetector gesture={anim.swipeGesture}>
        ...
        <View flex={1} height={40} position="relative">
          <Animated.View
            style={[styles.inputRow, anim.styles.inputGroup]}
