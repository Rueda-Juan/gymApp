...
import Animated, { useSharedValue, useAnimatedStyle, withTiming, interpolateColor, type SharedValue } from 'react-native-reanimated';
...
export const HistoryWorkoutCard = React.memo(function HistoryWorkoutCard({ item, index, onDelete }: HistoryWorkoutCardProps) {
  ...
  useEffect(() => {
    const delay = index * FADE_IN_STAGGER_MS;
    fadeOpacity.value = withTiming(1, { duration: FADE_IN_DURATION_MS + delay });
    fadeTranslateY.value = withTiming(0, { duration: FADE_IN_DURATION_MS + delay });
  ...
  return (
    <Animated.View style={[CARD_SHADOW_STYLE, fadeInStyle]}>
