...
import { TextInput as RNTextInput, Pressable, Platform, StyleSheet } from 'react-native';
import { FONT_SCALE } from '@/shared/ui/theme/tamagui.config';
import { Trash2, GripVertical, Plus, Minus, Link2, Unlink, ChevronDown, ChevronUp, ExternalLink } from 'lucide-react-native';
...
import { WORKOUT_LIMITS } from '@/shared/constants/workout';
...
const SURFACE_FALLBACK = '#1e1e1e';
const BORDER_FALLBACK = '#333';
...
export const RoutineExerciseRow: React.FC<RoutineExerciseRowProps> = React.memo(({
  ...
  // Resolver tokens de tema a valores concretos para pasarlos al Animated.View nativo
  const surfaceBg = (theme.surface?.val as string | undefined) ?? SURFACE_FALLBACK;
  const borderColorValue = isSuperset
    ? (theme.primary?.val as string | undefined) ?? PRIMARY_FALLBACK
    : (theme.borderColor?.val as string | undefined) ?? BORDER_FALLBACK;
  const borderWidth = isSuperset ? 2 : 1;
  ...
  useAnimatedReaction(
    () => isActive,
    (active) => {
      scale.value = active ? withSpring(motion.scale?.micro ?? 1, { ...motion.spring.snappy }) : withSpring(1, { ...motion.spring.snappy });
  ...
  return (
    <YStack marginBottom={isLinkedNext ? 0 : '$lg'} width="100%">
      ...
      <Animated.View
        testID="routine-exercise-item"
        style={[
          styles.container,
          { borderColor: borderColorValue, borderWidth, backgroundColor: surfaceBg },
          Platform.OS === 'android' && isActive ? styles.androidElevation : undefined,
        ...
        {isExpanded && (
          <YStack padding="$md" gap="$md" borderTopWidth={1} borderTopColor="$borderColor">
            ...
            {onViewDetails && (
              ...
              >
                <XStack alignItems="center" gap="$md" paddingVertical="$xs">
                  <AppIcon icon={ExternalLink} color="primary" size={18} />
                  <AppText variant="bodyMd" color="primary">Ver detalles / Editar</AppText>
                </XStack>
              </Pressable>
            )}
