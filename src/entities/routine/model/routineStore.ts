...
export interface LoadableRoutine {
  name: string;
  notes?: string | null;
  exercises?: {
    exerciseId?: string;
...
interface RoutineState {
  ...
  reorderExercises: (newExercises: RoutineEditorExercise[]) => void;
...
export const useRoutineStore = create<RoutineState>((set) => ({
  ...
  loadRoutine: (routine) => set({
    name: routine.name,
    notes: routine.notes || '',
    exercises: (routine.exercises || routine.routineExercises || []).map((e) => {
      ...
      if (hasNestedExercise) {
        return {
          ...
          sets: Number(e.targetSets ?? e.sets ?? DEFAULT_SETS),
      ...
      return {
        ...
        reps: String(e.reps ?? DEFAULT_REP_RANGE),
        supersetGroup: e.supersetGroup || null,
      };
    })
  }),
