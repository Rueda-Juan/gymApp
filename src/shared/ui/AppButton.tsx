      ...
      >
        <XStack alignItems="center" justifyContent="center" gap="$sm" flex={1} minWidth={0}>
          ...
          {label ? (
            <AppText
              variant="bodyMd"
              color={styleToken.textColor as ThemeColorKey}
...
export function SaveButton({
  ...
  const { Save } = require('lucide-react-native');
  const { AppIcon } = require('./AppIcon');

  return (
    <IconButton
      {...props}
      icon={loading ? <ActivityIndicator size="small" color="$primary" /> : <AppIcon icon={Save} color="primary" size={24} />}
      size={size}
      onPress={onPress}
