...
import { CableStackSelectorSheet } from '@/components/workout/plateSelector/CableStackSelectorSheet';
...
export const ExampleSheetIntegration: React.FC<ExampleSheetIntegrationProps> = ({
  ...
  if (loadType === 'barbell' || loadType === 'dumbbell') {
    const { createWeightEngine } = require('@/domain/weight/weightEngine');
    const engine = createWeightEngine(loadType, { barWeight: 20, availablePlates: [25,20,15,10,5,2.5,1.25] });
    const targetWeight = sets[selectedSetIndex]?.weight ?? 0;
    const result = engine.compute({ targetWeight });
    return (
  ...
  if (loadType === 'cable') {
    const { createWeightEngine } = require('@/domain/weight/weightEngine');
    const engine = createWeightEngine('cable', { unitWeight: 5, maxStack: 20 });
