...
class BarbellWeightEngine implements WeightEngine {
  constructor(private config: { barWeight: number; availablePlates: number[] }) {}
  compute({ targetWeight }: { targetWeight: number }) {
    return calculatePlates(targetWeight, this.config.barWeight, this.config.availablePlates);
  }
}

class CableWeightEngine implements WeightEngine {
  constructor(private config: { unitWeight: number; maxStack: number }) {}
  compute({ selectedBlocks }: { selectedBlocks: number }) {
    return {
      actualTotalWeight: selectedBlocks * this.config.unitWeight,
      blocks: selectedBlocks,
    };
  }
}

export function createWeightEngine(type: 'barbell' | 'dumbbell' | 'cable', config: any): WeightEngine {
  if (type === 'barbell' || type === 'dumbbell') {
    return new BarbellWeightEngine(config);
  }
  return new CableWeightEngine(config);
}
