...
export function useSupersetCarousel(
  ...
  useEffect(() => {
    ...
    setSupersetOrder(prev => {
      ...
      if (hasChanged) {
        try { logEvent('superset_order_changed', { groupIds }); } catch (e) { /* noop */ }
      }
  ...
  const rotateSupersetCarousel = useCallback(() => {
    ...
    try { logEvent('superset_rotate', { nextIndex, targetX }); } catch (e) { /* noop */ }
    runOnUI(() => {
      scrollTo(
        supersetScrollRef,
  ...
  const handleScrollEnd = useCallback(
    (e: NativeSyntheticEvent<NativeScrollEvent>) => {
      ...
      try { logEvent('superset_scroll_end', { newIndex, contentOffsetX: e.nativeEvent.contentOffset.x, carouselItemWidth }); } catch (e) { /* noop */ }
