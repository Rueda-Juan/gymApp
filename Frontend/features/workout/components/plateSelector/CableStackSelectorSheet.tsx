...
export const CableStackSelectorSheet = React.forwardRef<CableStackSelectorSheetRef, CableStackSelectorSheetProps>(
  (
    { value, onChange, maxStack, unitWeight, onClose },
    ref
  ) => {
          ...
          <YStack padding="$xl" gap="$lg">
            ...
            <YStack alignItems="center" gap="$md">
              <XStack alignItems="flex-end" gap={8}>
                {result.type === 'cable' && Array.from({ length: maxStack }).map((_, i) => (
                  <CableStackBlock
                    key={`block-${i}`}
                    index={i}
                    selected={i < value}
...
const CableStackBlock: React.FC<CableStackBlockProps> = React.memo(
  ({ index, selected, onPress, unitWeight, maxStack }) => {
    // Animation value should be stable per block
    const anim = React.useRef(useSharedValue(selected ? 1 : 0)).current;
    React.useEffect(() => {
    ...
    const theme = useTheme();
    // Accessibility: indicate position and total
    return (
      <Pressable
        onPress={onPress}
        accessibilityLabel={`Bloque ${index + 1} de ${maxStack}, ${selected ? 'seleccionado' : 'no seleccionado'}`}
        accessibilityState={{ selected }}
        style={{ alignItems: 'center' }}
           // Deshabilitar si el bloque anterior no está seleccionado
           disabled={typeof selected === 'number' ? index >= selected : false}
      >
        <Animated.View
          style={[
            {
              ...
              marginBottom: 2,
              borderRadius: 6,
              backgroundColor: selected ? theme.primary?.val : theme.surfaceSecondary?.val,
              borderWidth: 2,
