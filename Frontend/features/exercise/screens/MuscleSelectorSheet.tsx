...
import { HIERARCHICAL_MUSCLES, MUSCLE_LABELS, MuscleKey } from '@/constants/exercise';
...
export function MuscleSelectorSheet({
  ...
  const textColor = theme.color?.val ?? '#111';
  const textSecondary = theme.textSecondary?.val ?? '#888';
        ...
        {!expandedGroup && (
          <View style={styles.grid}>
            {HIERARCHICAL_MUSCLES.map((hm) => {
              const isSelected = activePending.includes(hm.category as any);
              const hasSubdivisions = !!hm.subdivisions?.length;
              return (
                <Pressable
                  key={hm.category}
                  onPress={() => hasSubdivisions ? openSubdivisions(hm.category) : toggleMuscle(hm.category as any)}
                  style={({ pressed }) => [
                    styles.muscleCard,
                    {
          ...
          <View>
            ...
            >
              ...
              <AppText variant="titleSm" fontWeight="700" color="color">
                {/* Puede arrojar undefined si MUSCLE_LABELS no tiene la clave. Validar claves en constants/exercise. */}
                {MUSCLE_LABELS[expandedGroup as keyof typeof MUSCLE_LABELS]}
              </AppText>
            </Pressable>
            ...
            <View style={styles.grid}>
              ...
              {(() => {
                const isWholeSelected = activePending.includes(expandedGroup as any);
                return (
                  <Pressable
                    onPress={() => { toggleMuscle(expandedGroup as any); goBack(); }}
                    style={({ pressed }) => [
                      styles.muscleCard,
              ...
              {expandedSubdivisions.map((sub) => {
                const isSelected = activePending.includes(sub as any);
                return (
                  <Pressable
                    key={sub}
                    onPress={() => toggleMuscle(sub as any)}
                    style={({ pressed }) => [
                      styles.muscleCard,
                      {
                        backgroundColor: isSelected ? primaryColor : surfaceSecondary,
