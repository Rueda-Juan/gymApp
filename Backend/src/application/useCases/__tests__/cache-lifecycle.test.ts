...
import type { ExerciseLoadCacheRepository, CachedWeightSuggestion } from '../../../domain/repositories/ExerciseLoadCacheRepository';
...
describe('Cache lifecycle integration', () => {
  ...
  let recordSetUseCase: RecordSetUseCase;
  ...
  beforeEach(() => {
    ...
    recordSetUseCase = new RecordSetUseCase(
      mockWorkoutRepo,
      mockStatsRepo,
      mockDb,
      mockLoadCacheRepo,
  ...
  it('invalidates cache when a new set is recorded', async () => {
    ...
    await recordSetUseCase.execute('w-1', {
      exerciseId: EXERCISE_ID,
      setNumber: 1,
      weight: 85,
      reps: 10,
