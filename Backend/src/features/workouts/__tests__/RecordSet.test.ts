...
describe('WorkoutService - RecordSet', () => {
  ...
  describe('happy path', () => {
    it('valida, registra el set y retorna set + newRecords', async () => {
      ...
      const result = await service.recordSet('w-1', VALID_SET_INPUT);
    ...
    it('ejecuta todo dentro de una única transacción', async () => {
      ...
      await service.recordSet('w-1', VALID_SET_INPUT);
  ...
  describe('validación de input', () => {
    it('rechaza input sin exerciseId', async () => {
      ...
      await expect(service.recordSet('w-1', noExerciseId)).rejects.toThrow();
    });
    ...
    it('rechaza exerciseId vacío', async () => {
      ...
      await expect(service.recordSet('w-1', emptyExerciseId)).rejects.toThrow();
    });

    it('rechaza peso negativo', async () => {
      const negativeWeight = { ...VALID_SET_INPUT, weight: -5 };

      await expect(service.recordSet('w-1', negativeWeight)).rejects.toThrow();
    });

    it('rechaza reps negativas', async () => {
      const negativeReps = { ...VALID_SET_INPUT, reps: -1 };

      await expect(service.recordSet('w-1', negativeReps)).rejects.toThrow();
    });

    it('rechaza setType inválido', async () => {
      const invalidSetType = { ...VALID_SET_INPUT, setType: 'superduper' };

      await expect(service.recordSet('w-1', invalidSetType)).rejects.toThrow();
    });

    it('rechaza rir fuera de rango (>10)', async () => {
      const badRir = { ...VALID_SET_INPUT, rir: 15 };

      await expect(service.recordSet('w-1', badRir)).rejects.toThrow();
    });

    it('rechaza rir negativo', async () => {
      const negRir = { ...VALID_SET_INPUT, rir: -1 };

      await expect(service.recordSet('w-1', negRir)).rejects.toThrow();
    });

    it('rechaza input null', async () => {
      await expect(service.recordSet('w-1', null)).rejects.toThrow();
    });

    it('rechaza input undefined', async () => {
      await expect(service.recordSet('w-1', undefined)).rejects.toThrow();
    });

    it('rechaza input que no es objeto (string)', async () => {
      await expect(service.recordSet('w-1', 'not an object')).rejects.toThrow();
    });

    it('rechaza input que no es objeto (number)', async () => {
      await expect(service.recordSet('w-1', 42)).rejects.toThrow();
    });
  });
  ...
  describe('boundary values', () => {
    it('acepta peso 0 (ejercicios de peso corporal)', async () => {
      ...
      const result = await service.recordSet('w-1', zeroWeight);
    ...
    it('acepta 0 reps (set abortado pero válido según schema)', async () => {
      ...
      const result = await service.recordSet('w-1', zeroReps);
    ...
    it('acepta rir = 0 (fallo muscular total)', async () => {
      ...
      const result = await service.recordSet('w-1', zeroRir);
    ...
    it('acepta rir = 10 (muy fácil)', async () => {
      ...
      const result = await service.recordSet('w-1', maxRir);
    ...
    it('acepta rir null (no reportado)', async () => {
      ...
      const result = await service.recordSet('w-1', nullRir);
  ...
  describe('async failures', () => {
    it('propaga error si addSet falla', async () => {
      ...
      await expect(service.recordSet('w-1', VALID_SET_INPUT)).rejects.toThrow('FK violation');
    });
    ...
    it('propaga error si getExerciseStats falla', async () => {
      ...
      await expect(service.recordSet('w-1', VALID_SET_INPUT)).rejects.toThrow('Stats table locked');
    });
    ...
    it('propaga error si upsertDailyStats falla', async () => {
      ...
      await expect(service.recordSet('w-1', VALID_SET_INPUT)).rejects.toThrow('Upsert failed');
    });
    ...
    it('no llama a repos si la validación falla (antes de la transacción)', async () => {
      ...
      await expect(service.recordSet('w-1', { weight: -1 })).rejects.toThrow();
