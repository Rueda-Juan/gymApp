...
describe('WorkoutService - RecordAllSets', () => {
  ...
  it('lanza ValidationError si el workout ya tiene sets registrados', async () => {
    ...
    await expect(service.recordAllSets('w-1', [])).rejects.toThrow(ValidationError);
  });
  ...
  it('acumula volumen de múltiples ejercicios en daily_stats', async () => {
    ...
    expect(mockStatsRepo.upsertDailyStats).toHaveBeenCalledWith(expect.objectContaining({
      totalVolume: 2000,
      totalSets: 11,
      totalReps: 110,
      workoutCount: 2,
